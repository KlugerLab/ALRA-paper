...
ClstAvgExp_facetted <- function(normData, label, labs, genes, geneGroup,methods,lowCol = "blue", highCol = "red", title = "", scale = 5){
  ...
  subLabel <- label[label %in% labs]
   ...
   avgData <- apply(subData, 1, FUN = function(x, xlabels){
    by(data = x, INDICES = xlabels, FUN = sum) /
      (by(data = x != 0.00, INDICES = xlabels, FUN = sum) + 0.0001)
  },xlabels = subLabel)
  avgData[avgData <0] <- 0
  avgData <- sweep(avgData,2, apply(avgData,2,max), '/')
  ...
  numData <- melt(apply(subData, 1, FUN = function(x, xlabels){
    by(data = x > 0.00, INDICES = xlabels, FUN = sum) / table(xlabels) * 100
  ...
  myggdata$genegroup <- factor(unlist(lapply( strsplit(as.character(myggdata$Gene),split = ".", fixed=T), function(x) x[2])), levels=genes)
...
#myggdata$genegroup <- geneGroup[myggdata$Gene]
  ...
  myggplot <- ggplot(data = myggdata ,aes(x = method, y = (Cluster), size = NumCells, col = AverageExpression)) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
    geom_point() + 
